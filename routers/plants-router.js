...
router.post('/plants/:id', async (req, res, next) => {
  try {
    ...
    if (!nickname) {
      return res.status(401).json({ message: "Please input plant's nickname" });
    }
    if (!water) {
      return res
        .status(401)
        .json({ message: "Please input plat's water frequency." });
    }
    const newPlant = await Plant.add({
      nickname,
      water: await nickname,
      water,
    });
